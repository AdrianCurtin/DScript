...
extern AudioDictionary curAudioDict; //defined in main.cpp
	...
	AudioDictionary::AudioDictionary()
	{
		...
		FMOD_System_SetOutput((FMOD_SYSTEM*)fmodsys, FMOD_OUTPUTTYPE_WINMM);
...
DWORD WINAPI audioRecord( LPVOID lpParam)
	{
		...
		sprintf(rcrdfile,"autoRecord_%i_%i_%i_%i_%i_%i.wav",timeinfo->tm_year-100,timeinfo->tm_mon+1,timeinfo->tm_mday,timeinfo->tm_hour,timeinfo->tm_min,timeinfo->tm_sec);
